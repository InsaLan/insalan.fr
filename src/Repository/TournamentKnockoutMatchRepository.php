...
class TournamentKnockoutMatchRepository extends NestedTreeRepository
{   
    ...
    public function generateMatches(TournamentKnockout $knockout, $players, $doubleElimination = false) {
        ...
        $root = new TournamentKnockoutMatch();
     ...
     * 
     * @param  TournamentKnockout $ko
     */
    public function propagateVictoryAll(TournamentKnockout $ko) {
     ...
     * 
     * @param  TournamentKnockoutMatch $koMatch
     */
    public function propagateFromNode(TournamentKnockoutMatch $koMatch) {
        ...
        foreach($children as $child) {
            ...
            $allChildrenEnded  &= $child->getMatch() !== null
                                && $child->getMatch()->getState() === TournamentMatch::STATE_FINISHED;
        }
        ...
        if($allChildrenEnded && // if children are ok
            ...
            if($match->getState() !== TournamentMatch::STATE_FINISHED) {
                if(!$match->getPart2()) {
     ...
     * @param  TournamentKnockoutMatch $koMatch
     */
    public function propagateVictory(TournamentKnockoutMatch $koMatch) {   
        $em = $this->getEntityManager();
        ...
        if($match === null || $match->getState() !== TournamentMatch::STATE_FINISHED || $parent === null)
            return;
        ...
        if($parentMatch === null) {
            $parentMatch = new TournamentMatch();
        ...
        if(!$loserMatch) {
            $loserMatch = new TournamentMatch();
     ...
     * Has a depth of 2
     *
     * @param  TournamentKnockout $ko [description]
     * @return [type]       [description]
     */
    public function getDepth(TournamentKnockout $ko) {
        $q = $this->createQueryBuilder('kom')
    ...
    public function getLeftDepth(TournamentKnockout $ko) {
     ...
     * @return String
     */
    public function getJson(TournamentKnockout $ko) {
        $teams = array();
    ...
    private function createChildren(TournamentKnockoutMatch $koMatch, $depth) {
        if($depth <= 0) return;
    ...
    private function createLoserChildren(TournamentKnockoutMatch $koMatch, $depth, $importLoser) {
        if($depth <= 0) {
    ...
    private function addChild(TournamentKnockoutMatch $parent, TournamentKnockoutMatch $child = null) {
        if(!$child)
            $child = new TournamentKnockoutMatch();
    ...
    private function setVictoryForPart1(TournamentMatch $match) {
        ...
        $r->setMatch($match);
        $match->addRound($r);
        $match->setState(TournamentMatch::STATE_FINISHED);
        $this->getEntityManager()->persist($r);
        $this->getEntityManager()->persist($match);
    }

    private function setVictoryForPart2(TournamentMatch $match) {
        $r = new TournamentRound();
        $this->getEntityManager()->persist($r);
        $this->getEntityManager()->persist($match);
        $this->getEntityManager()->flush();
