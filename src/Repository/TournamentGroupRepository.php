...
class TournamentGroupRepository extends EntityRepository
{
    ...
    public function getByStage(TournamentGroupStage $s)
    {
        $q = $this->getQueryBuilder();
    ...
    public function autoManageMatches(TournamentGroup $group)
    {

        $em = $this->getEntityManager();

        if ($group->getStatsType() == TournamentGroup::STATS_WINLOST) {
            ...
            if ($group->getMatches()->count() == 0) {
                $m = new TournamentRoyalMatch();
                $m->setState(TournamentMatch::STATE_UPCOMING);
                $m->setGroup($group);
                $group->addMatch($m);
            }
